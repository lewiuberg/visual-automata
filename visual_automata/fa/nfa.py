...
class VisualNFA:
    ...
    def table(self) -> DataFrame:
        ...
        all_transitions = self.__add_lambda(
            self.transitions,
            self.nfa.input_symbols
        )
        all_transitions = dict(sorted(all_transitions.items()))

        table: dict = {}
        for state, transition in all_transitions.items():
    ...
    # -------------------------------------------------------------------------
    # Adapt behavior of automata-lib NFA.

    # Works like NFA._get_next_current_state, without raising exceptions.
    ...
    #     """
    #     if input_symbol in self.nfa.transitions[current_state]:
    #         return self.nfa.transitions[current_state][input_symbol]
    def __get_lambda_closure(self, start_state):
        """
        ...
        while stack:
            ...
            if state not in encountered_states:
                ...
                if '' in self.nfa.transitions[state]:
                    stack.extend(self.nfa.transitions[state][''])

        return encountered_states

    def __get_next_current_states(self, current_states, input_symbol):
        """Return the next set of current states given the current set."""
        next_current_states = set()

        for current_state in current_states:
            symbol_end_states = self.nfa.transitions[current_state].get(
                input_symbol)
            if symbol_end_states:
                for end_state in symbol_end_states:
                    next_current_states.update(
                        self.__get_lambda_closure(end_state))
    ...
    @staticmethod
    def __transitions_pairs(all_transitions: dict) -> list:
        """
        ...
        for state, transitions in all_transitions.items():
            for symbol, transition in transitions.items():
                for trans in transition:
                    transition_possibilities.append((state, trans, symbol))
    ...
    def __transition_steps(
        ...
        for i, states in enumerate(current_states):
            current_state: list = []
            ...
            if len(current_state) == 1:
                current_states[i] = current_state.pop()
            else:
                current_states[i] = "{" + ",".join(current_state) + "}"
    ...
    @staticmethod
    def __add_lambda(all_transitions: dict, input_symbols: str) -> dict:
        """
    ...
    def input_check(
        ...
        current_state = self.nfa.initial_state
        ...
        for symbol in input_str:
            ...
            current_state = self.__get_next_current_states(
        ...
        final_transition = transitions_taken[-1]
        if final_transition.issubset(self.nfa.final_states):
            status = False
        else:
            status = True
    ...
    def show_diagram(
        ...
        *,
        ...
        for state in sorted(self.nfa.states):
            if (
                state in self.nfa.initial_state and state in
                self.nfa.final_states
            ):
        ...
        all_transitions_pairs = self.__transitions_pairs(self.nfa.transitions)
